...
import { MoreHorizontal, Pencil, Trash2, Calendar, FileText, UserCheck, Award } from 'lucide-react';
...
export default function ExaminationsPage() {
  const [tab, setTab] = useState<'schedules' | 'exams' | 'hall-tickets' | 'results'>('schedules');
  ...
  const scheduleColumns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <Link
            href={`/campus/examinations/schedules/${schedule.id}`}
    ...
    },
    {
      ...
      header: 'Type',
      cell: ({ row }) => <Badge variant="secondary">{row.getValue('examType')}</Badge>,
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => row.original._count?.exams || 0,
    },
    {
      accessorKey: 'status',
      header: 'Status',
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem asChild>
                <Link href={`/campus/examinations/schedules/${schedule.id}`}>
                  View Details
                </Link>
              </DropdownMenuItem>
              <DropdownMenuItem asChild>
                <Link href={`/campus/examinations/schedules/${schedule.id}/edit`}>
  ...
  const examColumns: ColumnDef<any>[] = [
    {
      ...
      header: 'Exam No.',
      cell: ({ row }) => <Badge variant="outline">{row.getValue('examNumber')}</Badge>,
    },
    {
    ...
    },
    {
      ...
      header: 'Exam Date',
      cell: ({ row }) => format(new Date(row.original.examDate), 'MMM dd, yyyy'),
    },
    {
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuLabel>Actions</DropdownMenuLabel>
              <DropdownMenuItem asChild>
                <Link href={`/campus/examinations/exams/${exam.id}`}>
                  View Details
                </Link>
              </DropdownMenuItem>
              <DropdownMenuItem asChild>
                <Link href={`/campus/examinations/exams/${exam.id}/edit`}>
                  <Pencil className="mr-2 h-4 w-4" />
                  Edit
                </Link>
              </DropdownMenuItem>
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuLabel>Actions</DropdownMenuLabel>
              <DropdownMenuItem asChild>
                <Link href={`/campus/examinations/hall-tickets/${hallTicket.id}`}>
                  View Details
                </Link>
              </DropdownMenuItem>
            </DropdownMenuContent>
      ...
      cell: ({ row }) => {
        ...
        return exam ? (
          <div>
            ...
            <p className="text-sm text-muted-foreground">{exam.courseOffering?.course?.name}</p>
          </div>
        ) : 'N/A';
      },
    },
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuLabel>Actions</DropdownMenuLabel>
              <DropdownMenuItem asChild>
                <Link href={`/campus/examinations/results/${result.id}`}>
                  View Details
                </Link>
              </DropdownMenuItem>
            </DropdownMenuContent>
  ...
  const filteredSchedules = schedules?.filter(
    (schedule) =>
      ...
      schedule.examType.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const filteredExams = exams?.filter(
    (exam) =>
      exam.examNumber.toLowerCase().includes(searchQuery.toLowerCase()) ||
      exam.courseOffering?.course?.name.toLowerCase().includes(searchQuery.toLowerCase())
  );
    ...
    <div className="container mx-auto py-10">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground mt-1">
          Manage exam schedules, hall tickets, and results
        </p>
      </div>
        ...
        <CardContent className="pt-6">
          <div className="grid gap-4 md:grid-cols-3">
            <div className="space-y-2">
              <Input
                placeholder={tab === 'schedules' ? 'Search schedules...' : tab === 'exams' ? 'Search exams...' : tab === 'hall-tickets' ? 'Search hall tickets...' : 'Search results...'}
                value={searchQuery}
      ...
      {tab === 'schedules' && (
        <>
          <div className="mb-4 flex justify-end">
            <Button asChild>
              <Link href="/campus/examinations/schedules/new">
      ...
      {tab === 'exams' && (
        <>
          <div className="mb-4 flex justify-end">
            <Button asChild>
              <Link href="/campus/examinations/exams/new">
                <FileText className="mr-2 h-4 w-4" />
                Schedule Exam
              </Link>
            </Button>
          ...
          <div className="mt-4 text-sm text-muted-foreground text-center">
            ...
            Total: {filteredExams?.length || 0} exams
          </div>
        </>
      )}
      ...
      {tab === 'hall-tickets' && (
        <>
          <div className="mb-4 flex justify-end">
            <Button asChild>
              <Link href="/campus/examinations/hall-tickets/new">
      ...
      {tab === 'results' && (
        <>
          <div className="mb-4 flex justify-end">
            <Button asChild>
              <Link href="/campus/examinations/results/new">
